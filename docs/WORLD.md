...
| Submarines off the quay | MCP servers that speak over stdio: no port, so no building, but running with tool access. One per server the scan found, holding station north of the quay with only the sail above water. It surfaces when the server starts and dives when the process exits. It never patrols: an idle server does not get to look busy |
