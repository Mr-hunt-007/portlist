...
def machine(m):
    ...
    snap = world.build(rows, hostinfo, m["groups"], {"engine": "docker" if m["containers"] else None,
                       ...
                       {"sessions": []}, sysinfo, [], outbound=m["islands"], conns=m["inbound"], traffic=m["traffic"])
    tasks = world.plan_pets(snap, [])
...
def main():
    ...
    data = {"generated": time.strftime("%Y-%m-%d"), "machines": [machine(devbox()), machine(web1())]}
